...
export default function Students() {
  ...
  useEffect(() => {
    getDocs(collection(database, "drivers"))
      .then((snapshots) => {
        let tmpStudentList = [];
  ...
  const handleDelete = (item) => {
    ...
    if (confirm) {
      deleteDoc(doc(database, "drivers", item))
        .then((res) => {
          setUpdateList(item);
        })
        .catch((err) => {
              ...
              {driversList &&
                driversList.map((item, index) => (
                  <tr key={item.id} className="bg-white border-b ">
                    ...
                    >
                      <Link href={"/drivers/"+item.id}>{index + 1} - {item.first_name} {item.last_name}</Link>
                    </th>
                    <td className="px-6 py-4">{item.gender}</td>
                    <td className="px-6 py-4">{item.student_id}</td>
                    <td className="px-6 py-4">{item.bus_id}</td>
                    <td className="px-6 py-4 flex">
                    <Link href={"/drivers/edit/"+item.id} className="text-[#7f0000] bg-[#fefcc0] hover:bg-white focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg p-2 text-center mx-3">Edit</Link>
                      <button
